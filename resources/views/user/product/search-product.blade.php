		...
		<div class="row">
			<div class="col-xs-12">
				<ul class="breadcrumb" itemscope itemtype="http://data-vocabulary.org/Breadcrumb">					
					...
					<li class="home">
						<a href="{{route('user.product-all')}}" ><span itemprop="title">Sản phẩm</span></a>					
					</li>
					

					<li>
						@if($key == 0 || $key == 1)
						<strong ><span itemprop="title">{{$search}}</span></strong>
						@endif
						@if($key == 2)
						<strong ><span itemprop="title">Sản phẩm mới</span></strong>
						@endif
						@if($key == 3)
						<strong ><span itemprop="title">Sản phẩm khuyến mãi</span></strong>
						@endif
						@if($key == 4)
						<strong ><span itemprop="title">Lọc sản phẩm</span></strong>
						@endif
					</li>
				...
				<div class="main-content collection">
					<div class="collection_header">
						<h1 class="title-head layout-title">
							@if($key == 0)
							<span>Sản phẩm danh mục - {{$search}}</span>
							@endif
							@if($key == 1)
							<span>Sản phẩm thương hiệu - {{$search}}</span>
							@endif
							@if($key == 2)
					...
					<div class="category-products products">
						...
						<div class="module-content">
							...
							<section class="products-view products-view-grid">
								<div class="row">
									@foreach($pro_search as $pr_search)
									<div class="col-xs-6 col-sm-4 col-md-4 col-lg-4 item" >
										<div class="product-box product-grid-item">
											<div class="product-thumbnail">
												@if($pr_search->listed_price == $pr_search->listed_price_sale)
												@else
												...
												@endif
												<a class="product-image" href="{{route('user.product-detail',['slug'=>$pr_search->slug,'id'=>$pr_search->id])}}" title="{{$pr_search->name}}">
													<picture style="padding: 30px">

														<source media="(min-width: 1200px)" srcset="{{url('/')}}/resources/views/admin/product/uploads/{{$pr_search->image_ava}}" />
															<source media="(min-width: 543px) and (max-width: 1199px)" srcset="{{url('/')}}/resources/views/admin/product/uploads/{{$pr_search->image_ava}}" />
																<source media="(min-width: 320px) and (max-width: 542px)" srcset="{{url('/')}}/resources/views/admin/product/uploads/{{$pr_search->image_ava}}" />
																	<img class="img-responsive" src="{{url('/')}}/resources/views/admin/product/uploads/{{$pr_search->image_ava}}" alt="{{$pr_search->name}}">

																</picture>
															</a>
															<a href="{{route('user.product-detail',['slug'=>$pr_search->slug,'id'=>$pr_search->id])}}" class="btn btn_detail">
														...
														<div class="product-info">
															...
															<h3 class="product-name">
																<a href="{{route('user.product-detail',['slug'=>$pr_search->slug,'id'=>$pr_search->id])}}" title="{{$pr_search->name}}">
																	{{$pr_search->name}}
																</a>
															</h3>
															<div class="price-box price-loop-style res-item">
																...
																<span class="special-price">
																	<span class="price">~{{$pr_search->listed_price_sale}}</span>
																</span>
																@if($pr_search->listed_price == $pr_search->listed_price_sale)
																@else
